...
def main():
    ...
    unet = UNet3DConditionModel.from_pretrained_2d(
        '../model_VTT_192_256_1030_fixbug/', subfolder="unet")
    # unet = UNet_EMASC.from_pretrained(
    #    'model_VTT_192_256_1023/checkpoint-62000', subfolder="unet", revision=args.non_ema_revision, # low_cpu_mem_usage=False
    # )
    ...
    for name, p in unet.named_parameters():
        ...
        # if 'fuse' in name or 'attn1' in name:
        if  ('temp' in name) and 'cloth' not in name:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            high_frequency_map = rearrange(high_frequency_map, "b f c h w -> (b f) c h w")
            pose = rearrange(pose, "b f c h w -> (b f) c h w")
            cloth_agnostic = rearrange(cloth_agnostic, "b f c h w -> (b f) c h w")
            dino_fea = rearrange(dino_fea, "b f l d -> (b f) l d")

            # print('111', image.shape, high_frequency_map.shape, pose.shape, cloth_agnostic.shape, dino_fea.shape)
            ...
            with accelerator.accumulate(unet):
                ...
                high_frequency_map = rearrange(high_frequency_map, "(b f) c h w -> b c f h w", f=video_length)
                ...
                # print('222', noisy_latents.shape, condition_latent.shape, encoder_hidden_states.shape)
                # Predict the noise residual and compute loss
                model_pred = unet(noisy_latents, condition_latent, timesteps, encoder_hidden_states).sample
                # print('3', model_pred.shape, target.shape)
